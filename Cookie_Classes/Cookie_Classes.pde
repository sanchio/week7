...
class Milk {
  ...
  Milk(String type, float amount, int daysUntilExpiration) {
    this.type = type;
    this.amount = amount;
    this.dayUntilExpiration = days; 
    
  }
  
  void drink(float howMuch) {
    if (this.amount > howMuch) {
   this.amount -= howMuch;
  println("You drank" + howMuch + "milk!");
  if(this.daysUntilExpiration<0) {
    println("It tasts terrible");
  }
    }
  }
...
class Cookie {
 ...
 class CookieJar {
   ...
   CookieJar() {
     for (int i = 0; i < 10; i++) {
       cookies.add(new Cookie("chocolate chip", "gigantic")) ;
     }
     
   
   Cookie takeCookie() {
     return cookies.remove(cookies.size()-1);
